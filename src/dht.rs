...
/// Interval for periodic maintenance (query progression, routing table upkeep).
...
async fn run(mut rpc: Rpc, mut receiver: mpsc::UnboundedReceiver<ActorMessage>) {
    ...
    loop {
        ...
        // Wait for a message, an incoming UDP packet, or the maintenance timer
        tokio::select! {
            biased;
            ...
            msg = receiver.recv() => {
                ...
                loop {
                    match receiver.try_recv() {
                        Ok(msg) => handle_actor_message(&mut rpc, msg, &mut put_senders, &mut get_senders),
                        Err(_) => break,
                    }
                }
            }
            ...
            packet = rpc.recv() => {
                if let Some((message, from)) = packet {
                    if let Some((target, response)) = rpc.process_message(message, from) {
        ...
        // Tick the RPC engine (query progression, routing table maintenance)
        let report = rpc.tick();

        // Flush packets generated by tick
        rpc.flush().await;

        // Process completed GET queries
