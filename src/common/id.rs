...
impl std::fmt::Display for DecodeIdError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            DecodeIdError::InvalidIdSize(e) => std::fmt::Display::fmt(e, f),
            DecodeIdError::OddNumberOfCharacters => {
                write!(
                    f,
                    "Hex encoding should contain an even number of hex characters"
                )
            }
            DecodeIdError::InvalidHexCharacter(s) => write!(f, "Invalid Id encoding: {s}"),
        }
    }
}
